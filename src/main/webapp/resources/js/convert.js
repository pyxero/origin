...
function WSCMobileSlide(arg_obj) {
	/*
	 * 参数: el是jQuery选择器 返回值: matrix.m41是x值 number matrix.m42是y值 number 参考链接
	...
	function touchDragMe($element) {
		...
		moveEle.on("touchmove", function(event) {
			...
			// 自由移动
			// moveEle.style.webkitTransform = 'translate3d(' + (curX) + 'px, '
			// + (curY) + 'px,0px)';
			// 只能移动Y轴方向
			// var realMoveEle = moveEle[0];
			moveEle[0].style.webkitTransform = 'translate3d(' + (curX) + 'px, '
					+ 0 + 'px,0px)';
